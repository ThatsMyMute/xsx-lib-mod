--[[
  ...
  Edits made by ThatsMyMute some for macsploit (I dick ride them currently) I want script-ware back 
  words from bungie#0001
...
local library = {
    version = "2.0.2",
    title = title or "xsx edit " .. tostring(math.random(1,366)),
    fps = 0,
    rank = "private"
}
...
function library:NewServer() -- same as rejoin just different server
    TeleportService:TeleportToPlaceInstance(library:GetPlaceId(), library:GetUserId())
end
function library:Copy(input) -- works if exploit has setclipboard ThatsMyMute was here
      setclipboard(input)
end
function library:Discord(input)    -- opens a link to discord invite :) you must only put invite code not discord.gg
    local res = syn.request or request({
    ...
    if res.Success then
        library:notification(library:GetUsername ' | joined discord', 3);
    end
...
function library:UnlockFps(new) -- fixed :) if it crashes that sucks for you
        setfpscap(new)
end
...
function library:Introduction()
    ...
    xsxLogo.Image = "http://www.roblox.com/asset/?id=3926305904"
    ...
    hashLogo.Image = "http://www.roblox.com/asset/?id=3926305904"
...
function library:Init(key)
    ...
    function TabLibrary:NewTab(title)
        ...
        function Components:NewLabel(text, alignment)
            ...
            label.Name = "label"
            label.Parent = page
            label.BackgroundColor3 = getgenv().settings.backgroundcolor
        ...
        function Components:NewSection(text)
            ...
            sectionFrame.BackgroundColor3 = getgenv().settings.backgroundcolor
